...
import time
import subprocess
# Import the PQE class from pq.py
from pq import PQE
...
try:
    ...
    exit(1)  # Exit the script if reading Excel file fails
...
def test_pxe_manufacturing(test_scenario, command_option):
    ...
    try:
        # Run the pq.py script with the specified option
        subprocess.run(['python3', 'pq.py', '-o', command_option], check=True)
    except subprocess.CalledProcessError as e:
        pytest.fail(f"Error running pq.py with option {command_option}: {e}")
    ...
    print("Sleeping done.")
    # Execute the pq.py script with the second command option (testpxe)
    try:
        subprocess.run(['python3', 'pq.py', '-o', 'testpxe'], check=True)
    except subprocess.CalledProcessError as e:
        pytest.fail(f"Error running pq.py with option 'testpxe': {e}")
